...
function createNotification(info)
	...
	real.Text = "welcome to epstein.wtf!"
...
do
	...
	cons[#cons + 1] = services.runService.RenderStepped:Connect(function(dt)
		...
		label.Text = `epstein<font color="{rgbstr(themes.preset.button)}">.wtf</font> beta{activeText}`
		local interpValue = rgbstr( themes.preset.button:Lerp(rgb(), math.abs(0.1 - 0.05 * cos(sine / 30))) )
		corner_text.Text = `epstein<font color="{interpValue}">.wtf</font> beta{activeText}`
	end)
end
...
local window = library:window({
	name = `epstein<font color="{rgbstr(themes.preset.button_alt)}">.wtf</font> {build_str}`,
...
if game.GameId == 113491250 then
	...
	if not getgenv().replacementForModules and not getrenv().shared.require then
		task.spawn(messagebox, 'Failed to attach into Phantom Forces environment.', 'epstein.wtf', bit32.bxor(32,0,256))
...
-- https://github.com/notpoiu/cobalt
			...
			if flags.chatlogs then
				if not Replication.getEntry(hitPlayer) then
					chatInterface.addMessage({
						isTeamChat = false;
						message = `<font color="{rgbstr(themes.preset.button)}">[epstein.wtf]</font> Invalid or unavalible ReplicationObject for {hitPlayer.DisplayName}`;
						senderData = {
							senderType = "Console";
						};
					})
				...
				task.spawn(function()
					...
					--[[xpcall(function()
						...
						if not why then
							chatInterface.addMessage({
								isTeamChat = false;
								message = `<font color="{rgbstr(themes.preset.button)}">[epstein.wtf]</font> Error calculating remaining health for {hitPlayer.DisplayName}. Reason: {reason}`;
								senderData = {
									senderType = "Console";
								};
							})
						...
						chatInterface.addMessage({
							isTeamChat = false;
							message = `<font color="{rgbstr(themes.preset.button)}">[epstein.wtf]</font> hit {hitPlayer.DisplayName} for {floor(damageTaken)} hp ({remainingHealth} remaining) in their {string.lower(damageAmount)}.`;
							senderData = {
								senderType = "Console";
							};
						})
						...
						chatInterface.addMessage({
							isTeamChat = false;
							message = `<font color="{rgbstr(themes.preset.button)}">[epstein.wtf]</font> hit {hitPlayer.DisplayName} for {floor(ogHealth)} hp (0 remaining) in their {string.lower(damageAmount)}.`;
							senderData = {
								senderType = "Console";
							};
						})
	...
	library.network:receive("bulletHitConfirm", function(hitPlayer, hitGroup, hitPos, damageAmount)
		...
		chatInterface.addMessage({
			isTeamChat = false;
			message = `<font color="{rgbstr(themes.preset.button)}">[epstein.wtf]</font> hit {hitPlayer.DisplayName} for {floor(damageAmount)} in their {string.lower(hitGroup)}. [id={id}]`;
			senderData = {
				senderType = "Console";
			};
		})
...
library.theme:add(section:colorpicker({name = "ui color", flag = "ui_color", color = themes.preset.button, alpha = 0, callback = function()
	...
	task.delay(1/5, function()
		...
		window:set_title(`epstein<font color="{rgbstr(themes.preset.button_alt)}">.wtf</font> {build_str} | {configName}`)
		for _, v in library.gui:GetDescendants() do
	...
	task.spawn(function()
		...
		function library:send_seraph_chat(message, username, hexColor)
			...
			channel:DisplaySystemMessage(
				`<font face="Arial" color='{rgbstr(themes.preset.button_alt)}'>[epstein.wtf]</font> <font color='{hexColor}' face="Arial">{username}:</font> {message}` -- <font color='{hexColor}'>{username}</font><font color='rgb(155,155,155)'>:</font> <font color='rgb(233,233,233)'>{message}</font>
			)
		end
